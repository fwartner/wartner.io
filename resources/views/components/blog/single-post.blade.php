<div>
    <figure>
        @if ($post->featured_image)
            <img class="w-full rounded-lg" src="{{ '/storage/' . $post->featured_image }}" alt="{{ $post->title }}">
        @else
        ...
        <div class="mx-auto mb-6 text-lg text-gray-600 lg:mb-0 lg:max-w-screen-xl">
            <h1 class="mx-0 mt-0 mb-3 text-5xl font-extrabold leading-none">
                <span class="block text-base font-semibold tracking-wide text-gray-600 uppercase"><time
                        ...
                        class="uppercase">{{ $post->created_at->format('F j, Y') }}</time></span>
                <span
                    class="block mt-2 text-3xl tracking-tight leading-8 sm:text-5xl sm:leading-none">{{ $post->title }}</span>
            </h1>

            <p class="pt-5 my-5 mx-0 prose">
                {!! $post->body !!}
            </p>
        </div>
    </div>
